...
async function scrapeGoogleMaps() {
    ...
    await page.goto('https://www.google.com/maps/search/عيادة+في+الرياض/');
    await delay(5000);
    
    let urls = new Set();
    
    console.log("Scrolling to collect 100 clinic links...");
    for (let i = 0; i < 40; i++) {
    ...
    for (let i = 0; i < urlsArray.length; i++) {
        try {
            console.log(`Visiting clinic ${i + 1}/${urlsArray.length}`);
            ...
            const name = nameEl ? await page.evaluate(el => el.innerText, nameEl) : "عيادة";
    ...
    const outPath = outDir + "\\\\عيادات_الرياض_100_lead.xlsx";
    xlsx.writeFile(wb, outPath);
    
    console.log("Done! File saved to: " + outPath);
    await browser.close();
}
